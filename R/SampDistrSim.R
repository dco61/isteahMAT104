...
SampDistr=function(popdata,N_Sample=1000,n=25){
  ...
  Samp.Distr.Stats=data.frame(
    Samp_Means=means,
	Samp_cil=cil,
	Samp_ciu=ciu,
	Samp_Median=medians,
    Samp_Var=variances,
	Samp_SD=stdeviations,
	Samp_MAD=mean_abs_dev,
    Std_Error=std.err.mean,
	Samp_Skewness=skew,
	Samp_Kurtosis=kurtos,
	Samp_JB=jb,
	Samp_Size=samp.size
  )
...
SampleAnalysis=function(DataIN){
	...
	#
	distr.median=c(round(mean(samp.medians),3),round(sd(samp.medians),3),round(popmed,3))
	#
	distr.var=c(round(mean(samp.variances),3),round(sd(samp.variances),3),round(popvar,3))
	#
	...
	#
	distr.skewness=c(round(mean(samp.skewness),3),round(sd(samp.skewness),3),round(popskewness,3))
	#
	distr.kurtosis=c(round(mean(samp.kurtosis),3),round(sd(samp.kurtosis),3),round(popkurtosis,3))
	#
	distr.jb=c(round(mean(samp.jb),3),round(sd(samp.jb),3),0)
	#
	col.names=c("Mean","Median","Variance","Std. Dev.","MAD","Std.Err.Mean","Skewness","Kurtosis")
	row.names=c("Mean","Std.Dev","Population")
	sampling.distr.stats=t(array(c(distr.mean,distr.median,distr.var,distr.stdev,distr.mad,distr.sem,distr.skewness,distr.kurtosis),dim=c(3,8),dimnames=list(row.names,col.names)))
	#
	# par(mfrow=c(2,1))
	hist(samp.means,breaks=20,freq=FALSE,main="Sampling Distribution of the Means",xlab="Sample Means")
	...
	#
	hist(samp.medians,breaks=20,freq=FALSE,main="Sampling distribution of Medians",xlab="Sample Medians")
	abline(v=popmed, lty=2, lwd=3, col="red")
	abline(v=distr.median[1], lty=3, lwd=3, col="blue")
	#
	# par(mfrow=c(4,1))
	hist(samp.variances,breaks=20,freq=FALSE,main="Sampling distribution of Variances",xlab="Sample Variances")
	abline(v=popvar, lty=2, lwd=3, col="red")
	abline(v=distr.var[1], lty=3, lwd=3, col="blue")
	#
	hist(samp.stdev,breaks=20,freq=FALSE,main="Sampling distribution of Standard Deviations",xlab="Sample Standard Deviations")
	abline(v=popsd, lty=2, lwd=3, col="red")
	abline(v=distr.stdev[1], lty=3, lwd=3, col="blue")
	#
	hist(samp.mad,breaks=20,freq=FALSE,main="Sampling distribution of Mean Abs Deviations",xlab="Sample MAD")
	abline(v=popmad, lty=2, lwd=3, col="red")
	abline(v=distr.mad[1], lty=3, lwd=3, col="blue")
	#	
	hist(samp.sem,breaks=20,freq=FALSE,main="Sampling distribution of Std. Error of the Mean",xlab="Sample Std. Error of the Mean")
	abline(v=popsem, lty=2, lwd=3, col="red")
	abline(v=distr.sem[1], lty=3, lwd=3, col="blue")
		#	
	hist(samp.skewness,breaks=20,freq=FALSE,main="Sampling distribution of Skewness",xlab="Sample Skewness")
	abline(v=popskewness, lty=2, lwd=3, col="red")
	abline(v=distr.skewness[1], lty=3, lwd=3, col="blue")
		#	
	hist(samp.kurtosis,breaks=20,freq=FALSE,main="Sampling distribution of Kurtosis",xlab="Sample Kurtosis")
	abline(v=popkurtosis, lty=2, lwd=3, col="red")
	abline(v=distr.kurtosis[1], lty=3, lwd=3, col="blue")
		#
	hist(samp.jb,breaks=20,freq=FALSE,main="Sampling distribution of Jarque-Bera Statistic",xlab="Sample JB Statistic")
#	abline(v=popkurtosis, lty=2, lwd=3, col="red")
