...
polyUrn = function(n=20, r=5, b=7, g=10, repl=FALSE){
  ...
  if(!testInt(r)){stop("r must be an integer!")}
  if(!testInt(b)){stop("b must be an integer!")}
  if(!testInt(g)){stop("g must be an integer!")}
  if(n>(r+b+g)){stop("Trying to draw more balls that urn content!")}
